...
func waitForCursor(c Console) error {
	...
	// Simulate human delay, this is to fix the issue on Linux when the expect answers too fast and the response is not
	...
	// `ESC[n;mR` in return before reading the answer. If the expect answers too fast (so the answer will be in between
